...
"%PY%" -m pip install torch torchaudio --index-url %TORCH_INDEX% --upgrade
if errorlevel 1 goto failed
...
if errorlevel 1 goto failed
REM Shared wheel cache for this run. Explicit --cache-dir overrides any user
REM pip.ini that sets no-cache-dir (e.g. NVIDIA PyIndex's autogenerated config),
REM so wheels downloaded once are reused across the 18 install passes instead
REM of re-fetched from PyPI each time. Lives under TEMP; safe to delete anytime.
if not defined PIP_CACHE set "PIP_CACHE=%TEMP%\stem-organizer-pip-cache"

echo.
echo [1/4] PyTorch (%TORCH_LABEL%) ...
"%PIP_PY%" -m pip install torch --index-url %TORCH_INDEX% -t "%DEST%" --upgrade --no-deps --cache-dir "%PIP_CACHE%"
if errorlevel 1 goto failed
REM torchvision (hear21passt/timm) + torchaudio (demucs) from same CUDA/CPU index
"%PIP_PY%" -m pip install torchvision torchaudio --index-url %TORCH_INDEX% -t "%DEST%" --upgrade --no-deps --cache-dir "%PIP_CACHE%"
if errorlevel 1 goto failed
REM Pillow: torchvision imports PIL; --no-deps above skips it
"%PIP_PY%" -m pip install Pillow -t "%DEST%" --upgrade --cache-dir "%PIP_CACHE%"
if errorlevel 1 goto failed
"%PIP_PY%" -m pip install filelock "typing-extensions>=4.10" "setuptools>=77" "sympy>=1.13.3" "networkx>=2.5.1" jinja2 "fsspec>=0.8.5" -t "%DEST%" --upgrade --cache-dir "%PIP_CACHE%"
if errorlevel 1 goto failed

echo [2/4] audio + UI deps ...
"%PIP_PY%" -m pip install "cffi>=1.16" -t "%DEST%" --only-binary=:all: --upgrade --cache-dir "%PIP_CACHE%"
if errorlevel 1 goto failed
"%PIP_PY%" -m pip install soundfile numpy sounddevice PySide6 "PySide6-Fluent-Widgets>=1.11,<2" psutil mutagen scipy librosa soxr audioread requests -t "%DEST%" --upgrade --cache-dir "%PIP_CACHE%"
if errorlevel 1 goto failed
"%PIP_PY%" -m pip install flac-detective==1.7.0 -t "%DEST%" --upgrade --cache-dir "%PIP_CACHE%"
if errorlevel 1 goto failed

echo [3/4] demucs ...
"%PIP_PY%" -m pip install omegaconf retrying submitit treetable cloudpickle colorama -t "%DEST%" --upgrade --cache-dir "%PIP_CACHE%"
if errorlevel 1 goto failed
"%PIP_PY%" -m pip install dora-search -t "%DEST%" --upgrade --no-deps --cache-dir "%PIP_CACHE%"
if errorlevel 1 goto failed
"%PIP_PY%" -m pip install lameenc "einops>=0.8.0" julius openunmix pyyaml tqdm -t "%DEST%" --upgrade --no-deps --cache-dir "%PIP_CACHE%"
if errorlevel 1 goto failed
"%PIP_PY%" -m pip install demucs -t "%DEST%" --upgrade --no-deps --cache-dir "%PIP_CACHE%"
if errorlevel 1 goto failed
...
"%PIP_PY%" -m pip install hear21passt timm --no-deps -t "%DEST%" --upgrade --cache-dir "%PIP_CACHE%"
if errorlevel 1 (
    echo ERROR: hear21passt pip install failed.
    echo Fix: check network / re-run install-deps.bat beside STEM-organizer.exe.
    goto after_inst
)
"%PIP_PY%" -m pip install pyyaml huggingface_hub safetensors packaging Pillow -t "%DEST%" --upgrade --cache-dir "%PIP_CACHE%"
...
if exist "%~dp0genre_gender_tagger\requirements.txt" (
    ...
    "%PIP_PY%" -m pip install -r "%~dp0genre_gender_tagger\requirements.txt" -t "%DEST%" --upgrade --cache-dir "%PIP_CACHE%"
...
"%PIP_PY%" -m pip install panns-inference -t "%DEST%" --upgrade --cache-dir "%PIP_CACHE%"
if errorlevel 1 (
...
"%PIP_PY%" -m pip install more-itertools tqdm librosa soxr -t "%DEST%" --upgrade --cache-dir "%PIP_CACHE%"
if errorlevel 1 (
...
if errorlevel 1 (
    ...
    "%PIP_PY%" -m pip install torchvision --index-url %TORCH_INDEX% -t "%DEST%" --upgrade --no-deps
    if errorlevel 1 (
...
if errorlevel 1 (
    ...
    "%PIP_PY%" -m pip install torchaudio --index-url %TORCH_INDEX% -t "%DEST%" --upgrade --no-deps
    if errorlevel 1 (
