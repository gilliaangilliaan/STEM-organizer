...
def show_genre_gender_help_dialog(parent: tk.Misc, mode: str) -> None:
    ...
    heading = (
        ...
        if is_genre
        else "Tag acapellas with voice gender + reverb"
    )
    intro = (
    ...
    sections = (
        ...
        (
            "Model",
            "Uses Essentia discogs-effnet-bs64 embeddings with two heads: "
            "gender-discogs-effnet (female/male) and "
            "nsynth_reverb-discogs-effnet (wet/dry), via TensorFlow .pb models. "
            "The .pb files ship in genre_gender_tagger\\models\\; no download "
            "is needed when those files are present.",
        ),
        (
            "Options",
